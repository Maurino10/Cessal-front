<template>
    <div class="p-5 custom-shadow rounded-lg">
        ...
        <div v-if="props.justificatifs !== null && props.justificatifs.length > 0" class="flex flex-wrap items-center gap-5">
            ...
            >
                <div class="relative flex flex-col items-center h-full gap-3 p-5 overflow-hidden border rounded-md group w-52">
                    ...
                    <div class="w-full">
                        <h4 class="text-sm text-center truncate">{{ justif.name }}</h4>
                    </div>
    
                    <div 
                        class="absolute top-0 left-0 items-center justify-center hidden w-full h-full gap-1 cursor-pointer group-hover:flex bg-slate-200/80 backdrop-blur-sm"
...
// Imports
    ...
    import magistratService from '@/services/cessions/magistratService.js'
...
// Functions
    ...
    const showAttachment = async (justif) => {
        try {
            const response = await magistratService.showCessionJustificatif(justif.id_cession, justif.id);
