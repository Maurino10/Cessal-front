<template>
    <VCardForm 
        title="Modification de la cour d'appel"
        subtitle="Modifiez les informations de la cour d'appel sélectionnée"
        width="600"
    >
        <template #card_text>
            <v-row>
...
// Functions
    ...
    const editRegion = async () => {
        ...
        try {
            const response = await caService.updateCA(idCA.value, form);
            
            setTimeout(() => {
                emit('reload');
                closeDialog();
                openSnackbar("Cour d\'appel mise à jour avec succès.", "success");
                openLoader(false);
